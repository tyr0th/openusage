...
final class OllamaProvider: ProviderRuntime {
    ...
    func hasLocalCredentials() async -> Bool {
        // Cheap, local-only probe (Keychain read, no network) mirroring the credential sources `probe()`
        // reads: either a browser session cookie or an API key counts as a usable local login.
        let authStore = self.authStore
        let credentials = await loadOffMainActor { authStore.loadCredentials() }
        return credentials.sessionCookie != nil || credentials.apiKey != nil
    }
